...
#ComposeSpec: {
	...
	{[=~"^x-" & !~"^(version|name|services|networks|volumes|secrets|configs)$"]: _}
	...
	#service: {
		...
		build?:  string | {
			...
			{[=~"^x-" & !~"^(context|dockerfile|args|ssh|labels|cache_from|cache_to|network|target|shm_size|extra_hosts|isolation|secrets)$"]: _}
		}
		blkio_config?: {
		...
		volumes?:     list.UniqueItems() & [...string | {
			...
			bind?: {
				...
				{[=~"^x-" & !~"^(propagation|create_host_path|selinux)$"]: _}
			}
			volume?: {
				nocopy?: bool
	...
	#service_config_or_secret: [...string | {
		source?: string
		target?: string
		uid?:    string
		gid?:    string
